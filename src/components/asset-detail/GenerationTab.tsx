...
import { Check, Dices, Info, Loader2, RotateCcw } from "lucide-react";
...
/** Génération tab: backend/source context (image profile) + the per-asset 3D
 *  generation override, pre-filled from the global defaults. Only values that
 *  differ from the defaults are persisted as the asset override. */
export function GenerationTab({
  ...
  project: string;
  asset: Asset;
  /** Image plan — null for text-to-3D assets (no image prerequisites). */
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Context moved from the old sticky bar: backend + image plan + source. */}
      {plan && (
        <>
          <div className="flex flex-col gap-1.5">
            ...
            <Select
              ...
              onValueChange={(v) =>
                updateAsset.mutate(
                  { assetId: asset.id, backend: v as Backend },
                  { onSuccess: () => toast.success("Backend mis à jour") },
                )
              }
              disabled={updateAsset.isPending || jobBusy}
            >
              <SelectTrigger className="h-8 w-full text-xs" aria-label="Backend 3D">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs">
            ...
            <Badge variant="secondary" className="font-mono">{asset.source}</Badge>
          </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {Object.keys(PRESETS).map((name) => (
          <Button key={name} variant="ghost" size="xs" onClick={() => applyPreset(name)}>
            {name}
          </Button>
        ))}
      </div>
          ...
          <div key={f.key} className="flex flex-col gap-1.5">
            <Label className="flex items-center gap-1 text-xs text-muted-foreground">
              ...
              <Tooltip>
                ...
                <TooltipContent className="max-w-[240px]">{f.hint}</TooltipContent>
              </Tooltip>
            </Label>
            <Input
              type="number"
