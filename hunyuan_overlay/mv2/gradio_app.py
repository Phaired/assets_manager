...
def _gen_shape(
    ...
    if MV_MODE:
        if mv_image_front is None and mv_image_back is None and mv_image_left is None and mv_image_right is None:
            ...
            # multiview path. (Re-applied by the installer's install_text3d step.)
...
def generation_all(
    ...
    tmp_time = time.time()
    # assets_gen patch: reduce to the target face count BEFORE texturing (not the
    # default 40k). For low-poly targets this is faster to texture (~8s vs ~15s,
    # CPU bake scales with face count), gives cleaner UVs, and avoids a second
    # texture-aware decimation downstream. HY_FACE_NUM overrides the target.
